...
func (m *MockUserDAO) GetUserByUsernameDAO(db *gorm.DB, username string) (*entity.User, error) {
...
func (m *MockUserDAO) GetUserByIdDAO(db *gorm.DB, id uint) (*entity.User, error) {
	args := m.Called(db, id)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	...
	}{
		{
			...
			setupMock: func(mockDAO *MockUserDAO, db *gorm.DB) {
				mockDAO.On("GetUserByUsernameDAO", db, "testuser").
					Return((*entity.User)(nil), gorm.ErrRecordNotFound) // 用户不存在
			...
			expectedMsg:    "成功",
			expectedCode:   result.SuccessCode,
			shouldPass:     true,
		},
		{
			name: "用户名已存在",
			setupMock: func(mock *MockUserDAO, db *gorm.DB) {
				mock.On("GetUserByUsernameDAO", db, "existuser").
					Return(&entity.User{Username: "existuser"}, nil) // 用户已存在
				// 不会调用 CreateUserDAO
			},
			inputDTO: &dto.RegisterDTO{
				Username: "existuser",
			...
			expectedStatus: http.StatusOK,
			expectedMsg:    "用户已存在",
			expectedCode:   result.FailedCode,
			shouldPass:     true,
		},
		{
			name: "数据库创建失败",
